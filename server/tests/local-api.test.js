...
const request = async (url, options = {}) => {
  const response = await fetch(`${origin}${url}`, {
    ...options,
    headers: { ...(options.body && !(options.body instanceof FormData) ? { 'content-type': 'application/json' } : {}), ...(cookie ? { cookie } : {}), ...options.headers },
  });
...
before(async () => {
  child = spawn(process.execPath, ['server/index.js'], {
    cwd: path.resolve('.'),
    env: { ...process.env, LOCAL_API_PORT: String(port), LOCAL_DATA_DIR: dataDir, LOCAL_LOGIN_USERNAME: 'tester', LOCAL_LOGIN_PASSWORD: 'secret' },
    stdio: 'ignore',
  });
...
test('serves public blugs with item-specific social preview metadata', async () => {
  ...
  assert.match(html, /property="og:title" content="Designing a Morning That Actually Belongs to You"/);
  ...
  assert.match(secondHtml, /property="og:title" content="Yoruba Culture: Cities, Oríkì, and a Living Artistic Tradition"/);
...
test('logs in with preset credentials and restores session', async () => {
  const login = await request('/api/auth/login', { method: 'POST', body: JSON.stringify({ username: 'tester', password: 'secret' }) });
...
test('creates sanitized posts and supports interactions', async () => {
  const created = await request('/api/posts', { method: 'POST', body: JSON.stringify({ title: 'Test', content: '<p>safe</p><script>bad()</script>' }) });
  assert.equal(created.response.status, 201);
  postId = created.body.post.id;
  assert.doesNotMatch(created.body.post.content, /script/);
  assert.equal((await request(`/api/posts/${postId}/likes`, { method: 'PUT' })).response.status, 204);
  assert.equal((await request(`/api/posts/${postId}/bookmarks`, { method: 'PUT' })).response.status, 204);
  assert.equal((await request(`/api/posts/${postId}/comments`, { method: 'POST', body: JSON.stringify({ body: 'Comment' }) })).response.status, 201);
  const asked = await request(`/api/posts/${postId}/paragraph-questions`, { method: 'POST', body: JSON.stringify({ paragraphIndex: 0, body: 'What does this point mean?' }) });
  assert.equal(asked.response.status, 201);
  assert.equal((await request(`/api/paragraph-questions/${asked.body.question.id}/answer`, { method: 'PATCH', body: JSON.stringify({ body: 'This is the blugger’s answer.' }) })).response.status, 200);
  const paragraphQuestions = (await request(`/api/posts/${postId}/paragraph-questions`)).body.questions;
...
test('seeds followable editorial channel profiles and their archives', async () => {
  ...
  assert.equal((await request('/api/social/follows/channel-history-ng', { method: 'PUT' })).response.status, 204);
...
test('creates support cases and exposes admin overview', async () => {
  const supportCase = await request('/api/support/cases', { method: 'POST', body: JSON.stringify({ subject: 'Help', body: 'Testing' }) });
  assert.equal(supportCase.response.status, 201);
  assert.equal((await request(`/api/support/cases/${supportCase.body.case.id}/messages`)).body.messages.length, 1);
