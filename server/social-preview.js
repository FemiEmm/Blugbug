...
export function renderPostPreview({ req, post, projectDir }) {
  ...
  const description = descriptionText.length > 190 ? `${descriptionText.slice(0, 187)}…` : descriptionText;
  const fallback = [...post.id].reduce((sum, character) => sum + character.charCodeAt(0), 0) % 2
    ? '/blug_default.png' : '/blug_default_2.webp';
  ...
  const metadata = `
    ...
    <meta property="og:image:alt" content="Cover image for ${escapeAttribute(post.title)}">
