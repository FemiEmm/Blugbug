...
const makeList = () => {
  restoreSelection();
  const selection = window.getSelection();
  if (!selection?.rangeCount || !surface.value) return;
  const range = selection.getRangeAt(0);
  if (range.collapsed) {
    const origin = range.startContainer instanceof Element ? range.startContainer : range.startContainer.parentElement;
    const block = origin?.closest('p,div');
    if (block && block !== surface.value && surface.value.contains(block)) {
