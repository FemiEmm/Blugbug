  ...
  <div class="page-shell">
    ...
    <main v-if="post" class="reader">
      ...
      <div class="reader-layout"><aside class="page-ad-rail"><AdSlot placement="sidebar"/><AdSlot placement="sidebar"/><AdSlot placement="sidebar"/></aside><section class="reader-main"><article class="story surface">
          ...
          <div ref="storyBody" class="story-body" :class="{ 'quote-mode': quoteMode }" @mouseup="captureQuote" @touchend="captureQuote"><p v-if="quoteMode" class="quote-instruction">Select the words you want to quote.</p><ParagraphQuestions :html="post.content" /></div>
        ...
        <LocalInteractions :postId="post.id" :selected-quote="selectedQuote" :quote-mode="quoteMode" @request-quote="beginQuote" @clear-quote="clearQuote" /></section>
...
const route=useRoute(),router=useRouter(),post=ref<LocalPost|null>(null),storyBody=ref<HTMLElement|null>(null),quoteMode=ref(false),selectedQuote=ref('');
...
const beginQuote=()=>{quoteMode.value=true;selectedQuote.value='';storyBody.value?.scrollIntoView({behavior:'smooth',block:'center'});};
const clearQuote=()=>{quoteMode.value=false;selectedQuote.value='';window.getSelection()?.removeAllRanges();};
const captureQuote=()=>{if(!quoteMode.value||!storyBody.value)return;const selection=window.getSelection();if(!selection||selection.isCollapsed||!selection.rangeCount)return;const range=selection.getRangeAt(0);if(!storyBody.value.contains(range.commonAncestorContainer))return;const text=selection.toString().replace(/\s+/g,' ').trim();if(text.length<3)return;selectedQuote.value=text.slice(0,300);quoteMode.value=false;window.setTimeout(()=>document.querySelector('.interactions')?.scrollIntoView({behavior:'smooth',block:'start'}),50);};
...
.story-body.quote-mode{cursor:text;outline:2px solid color-mix(in srgb,var(--orange) 45%,transparent);outline-offset:-2px}.quote-instruction{position:sticky;top:86px;z-index:3;width:max-content;max-width:100%;margin:0 auto 24px!important;padding:9px 14px;border-radius:999px;background:var(--orange);color:#fff;font:800 .76rem/1.2 Inter,ui-sans-serif,system-ui;box-shadow:var(--shadow)}.story-body.quote-mode :deep(::selection){background:color-mix(in srgb,var(--orange) 48%,transparent);color:var(--text)}
</style>
