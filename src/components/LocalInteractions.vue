<template>
  <section class="interactions">
    ...
    <div class="actions">
      ...
      <button :class="{ active: quoteMode || selectedQuote }" @click="$emit('request-quote')">❝ Quote</button>
    </div>
    ...
    <section v-if="selectedQuote" class="quote-composer"><header><span>Quoted from this blug</span><button type="button" aria-label="Remove quote" @click="$emit('clear-quote')">×</button></header><blockquote>“{{ selectedQuote }}”</blockquote><p>Add your opinion below.</p></section>
    <p v-else-if="quoteMode" class="selection-note">Highlight the passage you want to quote.</p>
    <form aria-label="Add a comment" @submit.prevent="submitComment"><label class="sr-only" for="new-comment">Write a comment</label><input id="new-comment" v-model="commentBody" maxlength="2000" :placeholder="selectedQuote ? 'Write your opinion about this quote' : 'Write a comment'" /><button :disabled="!commentBody.trim()||(!authStore.isAuthenticated.value&&guestName.length<2)">{{ selectedQuote ? 'Post opinion' : 'Comment' }}</button></form>
    <article v-for="comment in comments" :key="comment.id" class="comment">
      <button class="comment-author" type="button" :disabled="!comment.user_id" @click="openProfile(comment.user_id)"><strong>{{comment.user_id?'@':''}}{{ comment.chatter_name }}</strong><small v-if="!comment.user_id"> · Guest</small></button><blockquote v-if="comment.quote_text" class="comment-quote">“{{ comment.quote_text }}”</blockquote><p>{{ comment.body }}</p>
...
const props = defineProps<{ postId: string; selectedQuote?: string; quoteMode?: boolean }>();
...
const submitComment = async () => {if(!commentBody.value.trim())return;if(authStore.isAuthenticated.value)await addComment(props.postId,commentBody.value,props.selectedQuote);else{sessionStorage.setItem('blugbug-guest-name',guestName.value);await addGuestComment(props.postId,commentBody.value,guestName.value,await guestToken(),props.selectedQuote)}commentBody.value='';emit('clear-quote');await load()};
...
.selection-note{margin:18px 0 8px;padding:12px 14px;border-left:3px solid var(--orange);background:var(--soft);color:var(--text)}.quote-composer{margin-top:18px;padding:16px;border:1px solid var(--line);border-radius:14px;background:var(--panel)}.quote-composer header{display:flex;justify-content:space-between;align-items:center;color:var(--orange);font-size:.72rem;font-weight:900;text-transform:uppercase;letter-spacing:.08em}.quote-composer header button{min-height:30px;padding:0 9px;border:0;background:transparent;color:var(--muted);font-size:1.2rem}.quote-composer blockquote,.comment-quote{margin:12px 0;padding:10px 14px;border-left:3px solid var(--orange);background:var(--soft);color:var(--text);font:italic 1rem/1.55 Georgia,serif}.quote-composer>p{margin:8px 0 0;color:var(--muted);font-size:.78rem}.comment-quote{font-size:.95rem}
